...
//QAD_I2C_DutyCycle
//
//Enum used to define the clock duty cycle ratio to be used
enum QAD_I2C_DutyCycle : uint32_t {
	QAD_I2C_DutyCycle_2     = I2C_DUTYCYCLE_2,    //Normal duty cycle mode for speeds up to 100kHz
	QAD_I2C_DutyCycle_16_9  = I2C_DUTYCYCLE_16_9  //19:9 ratio duty cycle mode for speeds up to 400kHz
};
...
typedef struct {
	...
	uint32_t                    uClockSpeed;          //Clock speed for the peripheral to run at
                                                    //Low Speed mode is up to 100kHz
	                                                  //High Speed mode is up to 400kHz

	QAD_I2C_DutyCycle           eDutyCycle;           //Clock duty cycle to be used (member of QAD_I2C_DutyCycle enum)
...
private:
	...
	uint32_t                    m_uClockSpeed;           //Stores the clock speed being used

	QAD_I2C_DutyCycle           m_eDutyCycle;            //Stores the duty-cycle mode being used
	...
	IRQn_Type                   m_eIRQ_Error;            //The IRQ used by the I2C peripheral when Error interrupts are triggered
	                                                     //These are members of IRQn_Type defined in stm32f411xe.h

	I2C_HandleTypeDef           m_sHandle;               //Handle used by HAL functions to access I2C peripheral (defined in stm32f4xx_hal_i2c.h)
